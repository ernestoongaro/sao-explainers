import * as d3 from "https://cdn.jsdelivr.net/npm/d3@7/+esm";
      ...
      scenarios.forEach((scenario) => {
        ...
        controls
          ...
          .on("click", () => {
            if (svg) {
              exportDagAsPng(svg.node(), `${scenario.id}.png`);
            }
          });
        ...
        if (scenario.id === "column-aware-testing") {
          ...
          if (testNode) {
            const parentXs = Array.from(testNode.parents()).map((p) => p.x);
            const parentYs = Array.from(testNode.parents()).map((p) => p.y);
            const baselineX = parentXs.length
              ? Math.min(...parentXs)
              : testNode.x;
            const baselineY = parentYs.length
              ? Math.min(...parentYs)
              : testNode.y;
            const verticalOffset = LAYOUT_NODE_HEIGHT * 0.8;
            testNode.x = baselineX - verticalOffset;
            testNode.y = baselineY;
          }
        }
